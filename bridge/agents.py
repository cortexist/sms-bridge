...
front desk: any agent without a card speaks through it. "AGENTS", the single thread from
before agents had names, is retired.
"""
...
def lookup(key: str | None) -> dict | None:
    """By short id ("ides") or address ("ides@agents"). The legacy AGENTS address is
    retired: no identity, though is_agent() still keeps it off the radio."""
    if not key:
        return None
    k = key.strip().lower()
    if k == LEGACY.lower():
